...
dcorr_matrix = dcorr_returns.pivot(index='instrument_1', columns='instrument_2', values='distance_corr')
n = 50
dcorr_df_square = pd.DataFrame(dcorr_matrix,
                                 index=[f'{prices.columns.values[k]}' for k in range(1,n+1)],
                                 columns=[f'{prices.columns.values[k]}' for k in range(1,n+1)])
dcorr_matrix.index = [f'{prices.columns.values[k]}' for k in range(1,n+1)]
dcorr_matrix.columns = [f'{prices.columns.values[k]}' for k in range(1,n+1)]

plt.figure(figsize=(20,20))
sns.heatmap(dcorr_matrix, cmap='coolwarm', square=True, vmin=0.0, vmax=1.0)
...
def create_dcorr_matrix():
    prices = pd.read_csv('prices.csv')
    prices_shifted = prices.shift(5).dropna()
    prices = prices.iloc[5:]
    prices = prices
    for i, j in itertools.combinations(range(n_instruments), 2):
        dcorr = dcor.distance_correlation(prices.iloc[:, j].values, prices_shifted.iloc[:, i].values)
        print(prices.columns.values[i])
        results.append({'instrument_1': j, 'instrument_2_shifted': i, 'distance_corr': dcorr})
    dcorr_df = pd.DataFrame(results).sort_values('distance_corr', ascending=False)
    dcorr_df.to_csv('dcorr_df_no_algo.csv')


def create_dcorr_matrix_returns():
    prices_temp = prices.pct_change().dropna()
    prices_temp_2 = prices_temp.shift(50).dropna()
    prices_temp = prices_temp.iloc[50:]
    for i, j in itertools.combinations(range(n_instruments), 2):
        dcorr = dcor.distance_correlation(prices_temp_2.iloc[:, i].values, prices_temp.iloc[:, j].values)
...
def dcorr_resids():
    prices = pd.read_csv('prices.csv')
    y = prices['ALGO'].shift(1).dropna().values  # ALGO stays the predictor now
    prices = prices.drop('ALGO', axis=1)
    resid_df = pd.DataFrame()
    for inst in prices.columns:
        target = prices[inst].iloc[1:].values
        x = np.column_stack([np.ones(len(y)), y])
        coef, _, _, _ = np.linalg.lstsq(x, target, rcond=None)
        resid = target - x @ coef
        resid_df[f"{inst}_resid"] = resid
    for i in range(0,50):
        for j in range(0,50):
            if i == j:
                continue
            else:
                dcorr = dcor.distance_correlation(resid_df.iloc[:, j].values,resid_df.iloc[:, i].values)
                print(resid_df.columns.values[j])
                results.append({'instrument_1': i, 'instrument_2': j, 'distance_corr': dcorr})
    dcorr_df = pd.DataFrame(results).sort_values('distance_corr', ascending=False)
    dcorr_df.to_csv('dcorr_resids.csv')
