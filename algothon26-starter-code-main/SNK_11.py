...
The final revision adds CTGI/EELT and MTNS/BENI from the audited regime file.
They improved the portfolio across most chronological folds and remained
profitable across all nearby pair-parameter settings on the final 250 days.
The regime classifier itself is deliberately not included because its fitted
regime multipliers did not outperform a flat allocation out of sample.

Live books
----------
1. Ten disjoint rolling cointegration pairs.
...
PAIRS = (
    ...
    (31, 43),  # ACIX / ITPA
    (49, 50),  # MHRM / EAFC
    (25, 37),  # CTGI / EELT -- added after parameter/fold stability audit
    (33, 42),  # MTNS / BENI -- added after parameter/fold stability audit
)
...
STABLE_RULES = (
    (6, 8, 3, +1),       # OTCS <- HUXZ
    (24, 44, 40, -1),    # RRES <- HTRK
    (30, 44, 60, -1),    # RCRI <- HTRK
    (7, 40, 5, +1),      # HETT <- ULXY
    (16, 50, 1, +1),     # ANSO <- EAFC
    (26, 44, 120, +1),   # ALUT <- HTRK
    (12, 30, 1, +1),     # MSDP <- RCRI
    (49, 23, 2, -1),     # MHRM <- AGVF (ignored while pair active)
    (15, 15, 2, -1),     # HRET two-day reversal
    (46, 10, 60, +1),    # ILVX <- SMAH (ignored while pair active)
    (23, 26, 120, -1),   # AGVF <- ALUT
    (17, 3, 2, +1),      # DIHO <- SRNA
    (14, 15, 1, -1),     # CUBO <- HRET
    (38, 27, 120, +1),   # HRND <- ACAC
    (3, 44, 20, +1),     # SRNA <- HTRK
    (48, 30, 1, +1),     # FARS <- RCRI
    (43, 3, 1, +1),      # ITPA <- SRNA (ignored while pair active)
    (13, 5, 1, -1),      # EORC <- AMRP (ignored while pair active)
)
